$(document).ready(function () {
  ...
  let n = 0,
    ...
    wrongTries = 0,
    starTimer,
    totalWrongTries = JSON.parse(localStorage.getItem("hTotalWrongTries")) || 0,
    totalCorecktFlip =
      JSON.parse(localStorage.getItem("hTotalCorecktFlip")) || 0,
    bestTime = JSON.parse(localStorage.getItem("hBestTime")) || "-:-";
  ...
  function checkWin() {
    ...
    if (win.length == cards.length) {
      ...
      if (bestTime == "-:-" || x > 0) {
        localStorage.setItem("hBestTime", JSON.stringify(n));
      }
    }
  }
});
